...
test-notebooks:
	...
	for filename in *.ipynb; do \
		jupyter nbconvert --ExecutePreprocessor.startup_timeout=10 --ExecutePreprocessor.kernel_name=julia-$$(julia -e 'print(VERSION.major); print("."); print(VERSION.minor)') --ExecutePreprocessor.record_timing=False --to notebook --execute "$$filename" --output=out; \
		# remove "ipynb" extension to exclude this from loops in reruns in case the diff fails \
		mv out.ipynb out || exit 1; \
